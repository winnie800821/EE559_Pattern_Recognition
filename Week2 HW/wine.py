...
plt.scatter(training[label_train==1,0],training[label_train==1,1],marker="^",c="r",label="class1")
...
plt.scatter(training[label_train==2,0],training[label_train==2,1],marker="o",c="g",label="class2")
...
Y3mean=np.sum(training[label_train==3,1])/np.count_nonzero(label_train==3)
print('In wine_train,class3 Xmean is ',(X3mean))
print('In wine_train,class3 Ymean is ',(Y3mean))
plt.scatter(training[label_train==3,0],training[label_train==3,1],marker="+",c="b",label="class3")
...
for x in range(np.count_nonzero(label_train==1)):
 ...
 d3=math.sqrt((class1original[x,0]-X3mean)**2+(class1original[x,1]-Y3mean)**2)
 if d1<=d2 and d1<=d3:
...
for x in range(np.count_nonzero(label_train==2)):
 ...
 d3=math.sqrt((class2original[x,0]-X3mean)**2+(class2original[x,1]-Y3mean)**2)
 if d1>=d2 and d3>=d2:
...
for x in range(np.count_nonzero(label_train==3)):
 d1=math.sqrt((class3original[x,0]-X1mean)**2+(class3original[x,1]-Y1mean)**2)
 d2=math.sqrt((class3original[x,0]-X2mean)**2+(class3original[x,1]-Y2mean)**2)
 d3=math.sqrt((class3original[x,0]-X3mean)**2+(class3original[x,1]-Y3mean)**2)
 if d1>=d3 and d2>=d3:
...
for x in range(np.count_nonzero(label_test==1)):
 ...
 d3test=math.sqrt((class1originaltest[x,0]-X3mean)**2+(class1originaltest[x,1]-Y3mean)**2)
...
for x in range(np.count_nonzero(label_test==2)):
 ...
 d3test=math.sqrt((class2originaltest[x,0]-X3mean)**2+(class2originaltest[x,1]-Y3mean)**2)
...
for x in range(np.count_nonzero(label_test==3)):
 d1test=math.sqrt((class3originaltest[x,0]-X1mean)**2+(class3originaltest[x,1]-Y1mean)**2)
 d2test=math.sqrt((class3originaltest[x,0]-X2mean)**2+(class3originaltest[x,1]-Y2mean)**2)
 d3test=math.sqrt((class3originaltest[x,0]-X3mean)**2+(class3originaltest[x,1]-Y3mean)**2)
